...
import AnalisisDashboard from './Graficas'; // Import the provided AnalisisDashboard component
import { 
  faFileUpload, 
  faRocket, 
  faSpinner, 
...
export default function AnalizadorDatos() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setResultado(data);
      setData(data); // Pass the entire API response to AnalisisDashboard
      setActiveSection('informe');
    } catch (err) {
      console.error(err);
      ...
      <div className="h-full flex flex-nowrap overflow-x-auto">
        ...
        <div className="flex-shrink-0 w-full md:w-1/2 lg:w-2/3 h-full overflow-y-auto p-6 bg-gray-50">
          <div className="w-full text-left space-y-6">
            ...
            {activeSection === 'graficos' && data && <AnalisisDashboard data={data} />}
          </div>
        </div>
      </div>
    </div>
